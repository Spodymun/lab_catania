...
// Gravity constant used for converting g -> m/s²
static constexpr float GRAVITY_MS2 = 9.80665f;

// Magnetometer conversion factor:
// The sensor delivers values in units of "one earth magnetic field" scaled by 1e-3.
// Assuming the raw unit after scaling is Gauss, convert to Tesla: 1 Gauss = 1e-4 Tesla.
// => combined factor applied AFTER the 1e-3 scaling: multiply by 1e-4
// If your sensor delivers mT instead of Gauss, change this to 1e-3.
static constexpr float MAG_TO_TESLA = 1e-4f;
        ...
        switch(header.cmd){                                 // Step 3.2: header.cmd tells what kind of data is inside the payload.
            ...
            case EP_CMD_Raw_GYRO_ACC_MAG_:{
                ...
                if(EP_SUCC_ == eOD.Read_Ep_Raw_GyroAccMag(&ep_Raw_GyroAccMag)){
                    // NOTE: Check your sensor datasheet for the raw packet units.
                    // If acc[] is in g, multiply by GRAVITY_MS2 to get m/s² (ROS2 standard).
                    // If acc[] is already in m/s², remove the GRAVITY_MS2 factor.
                    float gyro_x = ep_Raw_GyroAccMag.gyro[0]; // rad/s — already SI
                    ...
                    // *** UNIT CONVERSION: Gauss -> Tesla (1 Gauss = 1e-4 Tesla) ***
                    float mag_x = ep_Raw_GyroAccMag.mag[0] * MAG_TO_TESLA;
                    float mag_y = ep_Raw_GyroAccMag.mag[1] * MAG_TO_TESLA;
                    float mag_z = ep_Raw_GyroAccMag.mag[2] * MAG_TO_TESLA;

                    imu_data_msg.angular_velocity.x =  gyro_x;
                    imu_data_msg.angular_velocity.y =  gyro_y;
                    imu_data_msg.angular_velocity.z =  gyro_z;

                    imu_data_msg.linear_acceleration.x = acc_x; // m/s²
                    imu_data_msg.linear_acceleration.y = acc_y;
                    imu_data_msg.linear_acceleration.z = acc_z;

                    imu_data_mag_msg.magnetic_field.x = mag_x; // Tesla
            ...
            case EP_CMD_RPY_:{
                ...
                if(EP_SUCC_ == eOD.Read_Ep_RPY(&ep_RPY)){        //Another Example reading of the received Roll Pitch and Yaw
                    // *** UNIT CONVERSION: degree -> radian (ROS2 standard) ***
                    float roll  = ep_RPY.roll  * (M_PI / 180.0f);
                    float pitch = ep_RPY.pitch * (M_PI / 180.0f);
                    float yaw   = ep_RPY.yaw   * (M_PI / 180.0f);
            ...
            case EP_CMD_COMBO_: {
                ...
                if(EP_SUCC_ == eOD.Read_Ep_Combo(&ep_Combo)){
                    // Accelerometer:
                    // Raw unit: g (1g = 9.80665 m/s²) -> convert to m/s² for ROS2
                    float ax = (ep_Combo.ax)*(1e-5f) * GRAVITY_MS2;  // m/s²
                    float ay = (ep_Combo.ay)*(1e-5f) * GRAVITY_MS2;
                    float az = (ep_Combo.az)*(1e-5f) * GRAVITY_MS2;
                    // Gyroscope:
                    float wx = (ep_Combo.wx)*(1e-5f); // rad/s — already SI
                    float wy = (ep_Combo.wy)*(1e-5f);
                    float wz = (ep_Combo.wz)*(1e-5f);
                    // Magnetometer:
                    // Raw unit after 1e-3 scaling: assumed Gauss -> convert to Tesla (* 1e-4)
                    float mx = (ep_Combo.mx)*(1e-3f) * MAG_TO_TESLA; // Tesla
                    float my = (ep_Combo.my)*(1e-3f) * MAG_TO_TESLA;
                    float mz = (ep_Combo.mz)*(1e-3f) * MAG_TO_TESLA;
                    // Quaternion in (q1,q2,q3,q4)=(w,x,y,z) format — dimensionless, no conversion needed
                    ...
                    // RPY:
                    // *** UNIT CONVERSION: degree -> radian (ROS2 standard) ***
                    float roll  = (ep_Combo.roll)*(1e-2f)  * (M_PI / 180.0f); // rad
                    float pitch = (ep_Combo.pitch)*(1e-2f) * (M_PI / 180.0f);
                    float yaw   = (ep_Combo.yaw)*(1e-2f)   * (M_PI / 180.0f);

                    imu_data_msg.angular_velocity.x =  wx;
                    imu_data_msg.angular_velocity.y =  wy;
                    imu_data_msg.angular_velocity.z =  wz;

                    imu_data_msg.linear_acceleration.x = ax; // m/s²
                    ...
                    imu_data_rpy_msg.magnetic_field.x = roll;  // rad
                    imu_data_rpy_msg.magnetic_field.y = pitch;
                    imu_data_rpy_msg.magnetic_field.z = yaw;

                    imu_data_mag_msg.magnetic_field.x = mx; // Tesla
